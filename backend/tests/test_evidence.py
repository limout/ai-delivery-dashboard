...
def test_build_adds_wip_and_blocked_evidence():
    ...
    assert evidence["evidence_type"] == "delivery_work_items"
    ...
    assert evidence["wip_items"][0]["evidence_type"] == "work_item_age"
    assert evidence["wip_items"][0]["source"] == "jira"
...
def test_completed_item_is_not_wip_evidence():
    ...
    assert evidence["wip_count"] == 0
    assert evidence["blocked_count"] == 0
    assert evidence["wip_items"] == []
    assert evidence["blocked_items"] == []


def test_evidence_limits_items_to_five():
    work_items = [
        item(f"KAN-{index}", "In Progress")
    ...
    assert evidence["wip_count"] == 7
    assert len(evidence["wip_items"]) == 5